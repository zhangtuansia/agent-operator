 ...
 * Deep Link Handler
 *
 * Parses agentoperator:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   agentoperator://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   agentoperator://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   agentoperator://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   agentoperator://sources[/source/{sourceSlug}]          - Sources list
 *   agentoperator://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   agentoperator://action/{actionName}[/{id}][?params]
 *   agentoperator://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   agentoperator://allChats/chat/abc123                   (specific chat)
 *   agentoperator://settings/shortcuts                     (shortcuts page)
 *   agentoperator://sources/source/github                  (github source info)
 *   agentoperator://action/new-chat                        (uses active window)
 *   agentoperator://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   agentoperator://workspace/ws123/allChats/chat/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., agentoperator://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., agentoperator://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // agentoperator://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // agentoperator://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // agentoperator://action/... (no workspace - uses active window)
    if (host === 'action') {
