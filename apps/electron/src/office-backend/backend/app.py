...
# PyInstaller compatibility: use _MEIPASS for bundled resources (read-only),
# writable state goes to ~/.cowork/office/
_IS_FROZEN = getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS')
if _IS_FROZEN:
    ROOT_DIR = sys._MEIPASS  # PyInstaller extracted data directory (read-only)
    _DATA_DIR = os.path.join(os.path.expanduser("~"), ".cowork", "office")
    os.makedirs(_DATA_DIR, exist_ok=True)
else:
    ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _DATA_DIR = ROOT_DIR
MEMORY_DIR = os.path.join(os.path.dirname(ROOT_DIR), "memory") if not _IS_FROZEN else os.path.join(_DATA_DIR, "memory")
...
AGENTS_STATE_FILE = os.path.join(_DATA_DIR, "agents-state.json")
JOIN_KEYS_FILE = os.path.join(_DATA_DIR, "join-keys.json")
