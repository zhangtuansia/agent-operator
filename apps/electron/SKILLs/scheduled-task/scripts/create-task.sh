#!/bin/bash
# Create a scheduled task via Cowork internal API.
...
#
# Environment variables (set automatically by Cowork cowork session):
#   COWORK_API_BASE_URL - Internal proxy URL (always points to local proxy)
...
resolve_http_node_runtime() {
  ...
  if [ -n "${COWORK_ELECTRON_PATH:-}" ] && [ -x "${COWORK_ELECTRON_PATH}" ]; then
    HTTP_NODE_CMD="$COWORK_ELECTRON_PATH"
    HTTP_NODE_ARGS=()
...
if [ -z "$COWORK_API_BASE_URL" ]; then
  echo '{"success":false,"error":"COWORK_API_BASE_URL not set. This script must run inside a Cowork cowork session."}'
  exit 1
fi
...
# COWORK_API_BASE_URL always points to the local proxy: http://127.0.0.1:PORT
BASE_URL="${COWORK_API_BASE_URL%/}"
