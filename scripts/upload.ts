  ...
  if (uploadElectron) {
    ...
    for (const yml of yamlManifests) {
      ...
      if (existsSync(ymlPath)) {
        ...
        missingManifests.push(yml.file);
      }
    }

    if (missingManifests.length > 0) {
      console.warn(`\n⚠️  WARNING: Missing YAML manifests for electron-updater auto-update:`);
      for (const f of missingManifests) {
        console.warn(`   - ${f}`);
      }
      console.warn(`   These are generated by electron-builder with --publish always flag.`);
      console.warn(`   Without them, auto-update will NOT work for those platforms.\n`);
    }
    ...
    if (existsSync(macArmZip)) {
      ...
      missingZips.push('Cowork-arm64.zip');
    }
    if (existsSync(macX64Zip)) {
      await uploadFile(macX64Zip, `electron/latest/${basename(macX64Zip)}`);
    } else {
      missingZips.push('Cowork-x64.zip');
    }

    if (missingZips.length > 0) {
      console.warn(`\n⚠️  WARNING: Missing macOS ZIPs for auto-update:`);
      for (const f of missingZips) {
        console.warn(`   - ${f}`);
      }
      console.warn(`   macOS auto-update requires ZIP files. Build with: electron-builder --mac --publish always\n`);
    }
