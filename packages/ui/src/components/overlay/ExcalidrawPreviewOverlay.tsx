/**
 * ExcalidrawPreviewOverlay — fullscreen diagram preview with zoom and pan.
 *
 * Renders a pre-rendered Excalidraw SVG string in a zoomable/pannable viewport
...
import { PenTool, RotateCcw, Minus, Plus, Check } from 'lucide-react'
...
export interface ExcalidrawPreviewOverlayProps {
  isOpen: boolean
  onClose: () => void
  /** Pre-rendered SVG string from Excalidraw export */
  svg: string
  /** Original Excalidraw source (for copy button) */
  code: string
}
...
export function ExcalidrawPreviewOverlay({
  ...
  // Fit-to-viewport on open for a better default experience.
  // Uses requestAnimationFrame so the container has a measured size.
  useEffect(() => {
    if (!isOpen) return
