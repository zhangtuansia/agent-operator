...
import { renderMermaid, parseMermaid } from '../index.ts'
...
describe('layoutGraph – two disconnected subgraphs', () => {
  it('renders without overlap in LR direction', async () => {
    ...
    const result = await layoutGraph(parsed)
  ...
  it('renders without overlap in TD direction', async () => {
  ...
  it('respects direction for stacking (LR = vertical)', async () => {
  ...
  it('respects direction for stacking (TD = horizontal)', async () => {
...
describe('layoutGraph – multiple disconnected components', () => {
  it('renders three disconnected subgraphs without overlap', async () => {
    const source = `graph LR
      subgraph A [Alpha]
        A1 --> A2
      end
  ...
  it('renders five disconnected nodes without overlap', async () => {
...
describe('layoutGraph – mixed connected and disconnected', () => {
  it('renders two connected subgraphs + one disconnected', async () => {
    const source = `graph LR
  ...
  it('renders connected nodes + isolated node', async () => {
    const source = `graph LR
      A --> B --> C
      D[Isolated]`
...
describe('layoutGraph – quality preservation', () => {
  it('each component looks identical to standalone rendering', async () => {
    ...
    const standaloneResult = await layoutGraph(standaloneParsed)
    ...
    const combinedResult = await layoutGraph(combinedParsed)
...
describe('layoutGraph – disconnected edge cases', () => {
  it('handles single node as its own component', async () => {
    const source = `graph LR
      A --> B
      C[Isolated]`
  ...
  it('handles empty subgraph with disconnected nodes', async () => {
    const source = `graph LR
      subgraph Empty
      end
      A --> B`
  ...
  it('handles subgraph containing entire component', async () => {
    const source = `graph LR
      subgraph Component1
        A --> B --> C
      end
...
describe('renderMermaid – disconnected components', () => {
  it('renders two disconnected subgraphs to valid SVG', async () => {
    const source = `graph LR
      subgraph Today [Today]
        A --> B
      end
  ...
  it('renders isolated nodes to valid SVG', async () => {
    const source = `graph LR
      A[First]
      B[Second]
      C[Third]`
