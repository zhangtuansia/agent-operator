...
import { getShapeAttachmentPoint, getShapeDimensions } from './shapes/index.ts'
 ...
 * Uses shape-aware attachment point calculation for proper edge alignment.
 */
function getNodeAttachmentPoint(
  ...
  const dimensions = getShapeDimensions(node.shape, node.displayLabel, {
    useAscii: graph.config.useAscii,
    padding: graph.config.boxBorderPadding,
  })
  const baseCoord = node.drawingCoord!
  return getShapeAttachmentPoint(node.shape, dir, dimensions, baseCoord)
}
...
function drawBundledEdgeSegment(
  ...
  for (let i = 1; i < drawingPath.length; i++) {
    ...
    if (!drawingCoordEquals(from, to)) {
      // Use offset 1 for first segment (skip the border character), 0 otherwise
      const offsetFrom = i === 1 && bundle.type === 'fan-in' ? 1 : 0
      // Use offset -1 for last segment (skip the border character), 0 otherwise
      const offsetTo = i === drawingPath.length - 1 && bundle.type === 'fan-out' ? -1 : 0
...
function drawBundleSharedPath(graph: AsciiGraph, bundle: EdgeBundle): [Canvas, Canvas] {
  ...
  for (let i = 1; i < drawingPath.length; i++) {
    ...
    const to = drawingPath[i]!
    if (!drawingCoordEquals(from, to)) {
      // Use offset 1 for first segment from source (fan-out)
      const offsetFrom = i === 1 && bundle.type === 'fan-out' ? 1 : 0
      // Use offset -1 for last segment to target (fan-in)
      const offsetTo = i === drawingPath.length - 1 && bundle.type === 'fan-in' ? -1 : 0
      drawLine(pathCanvas, from, to, offsetFrom, offsetTo, useAscii, style)
    }
  }
...
function drawBundleArrowhead(graph: AsciiGraph, bundle: EdgeBundle): Canvas {
  ...
  // Get drawing coord at the target node's actual border (not grid cell center)
  const graphDir = graph.config.graphDirection
  const entryDir = graphDir === 'TD' ? Up : Left
