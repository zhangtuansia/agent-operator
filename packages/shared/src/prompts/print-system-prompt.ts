#!/usr/bin/env bun
/**
 * Debug script to print the full Cowork system prompt with annotations.
...
printAnnotation('  2. Cowork Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.cowork/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@cowork/electron/main.log' },
  '/Users/example/.cowork/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.cowork/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by OperatorAgent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Cowork additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Cowork Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/operator-agent.ts    ${colors.dim}// User message building${colors.reset}
